                                    ...
                                    <div class="d-flex gap-2">
                                       <button type="button" 
                                                ...
                                                onclick="abrirModalVerReserva({
                                                    ...
                                                    apellidos: '{{ $reserva->cliente->persona->apellidos ?? '' }}',
                                                    nombre_mascota: '{{ $reserva->mascota->nombre ?? '' }}',
                                                    especie: '{{ $reserva->mascota->especie ?? '' }}',
                                                    vacuna: '{{ $reserva->vacuna ? 'Sí' : 'No' }}',
                                        ...
                                        @if($reserva->estado == 'P')
                                            ...
                                            <button type="button" 
                                                    ...
                                                    onclick="abrirModalAtencion(
                                                        ...
                                                        '{{ ($reserva->cliente->persona->nombres ?? '') }} {{ ($reserva->cliente->persona->apellidos ?? '') }}',
                                                        '{{ $reserva->vacuna ? 'Sí' : 'No' }}',
                                                        '{{ $reserva->enfermedad ? 'Sí' : 'No' }}',
                                                        '{{ $reserva->mascota->descripcion_alergia ?? 'Sin información' }}',
                                                        '{{ $reserva->mascota->alergia ?? 'Ninguna' }}'
                                                    )">
...
<div class="modal fade" id="modalAtencion" tabindex="-1" aria-labelledby="modalAtencionLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
            ...
            <form action="{{ route('empleado.reservas.guardarAtencion') }}" method="POST">
                @csrf
